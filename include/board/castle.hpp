...
template <Colour C> constexpr Square CastleMask::rook_dst(bool queen_side) noexcept {
  constexpr Square ROOK_DST_MAP[4] = {F1, D1, F8, D8};
  return ROOK_DST_MAP[index(C, queen_side)];
}

template <Colour C> constexpr Square CastleMask::king_dst(bool queen_side) noexcept {
