...
public:
    ...
    template <Colour C>
    constexpr Square rook_src(bool queen_side) const noexcept;
    template <Colour C>
    static constexpr Square rook_dst(bool queen_side) noexcept;
    template <Colour C>
    static constexpr Square king_dst(bool queen_side) noexcept;
...
template <Colour C>
constexpr Square CastleMask::rook_dst(bool queen_side) noexcept {
    constexpr Square ROOK_DST_MAP[4] = {F1, D1, F8, D8};
    return ROOK_DST_MAP[index(C, queen_side)];
}

template <Colour C>
constexpr Square CastleMask::king_dst(bool queen_side) noexcept {
...
inline std::string CastleMask::to_str(Castle cr, bool chess960) const {
    ...
    if (!cr) out << "-";
    if (cr & WhiteOO) out << (chess960 ? Lyra::to_str(file_of(rook_sq[0])) : "K");
    if (cr & WhiteOOO) out << (chess960 ? Lyra::to_str(file_of(rook_sq[1])) : "Q");
    if (cr & BlackOO) out << (chess960 ? Lyra::to_str(file_of(rook_sq[2])) : "k");
    if (cr & BlackOOO) out << (chess960 ? Lyra::to_str(file_of(rook_sq[3])) : "q");
    return out.str();
}
