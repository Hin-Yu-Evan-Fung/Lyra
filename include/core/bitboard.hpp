...
template <Direction D> constexpr BB shift(BB b) {
  ...
  case S:
    return b >> 8;
  case E:
    return (b & ~from(FileH)) << 1;
  case W:
    return (b & ~from(FileA)) >> 1;
  case NE:
    return (b & ~from(FileH)) << 9;
  case NW:
    return (b & ~from(FileA)) << 7;
  case SE:
    return (b & ~from(FileH)) >> 7;
  case SW:
    return (b & ~from(FileA)) >> 9;
  }
}
...
template <Colour C> constexpr BB attack_bb(PieceType att, Square sq, BB occ) {
  ...
  case B:
    return BISHOP_ATK[sq][occ];
  case R:
    return ROOK_ATK[sq][occ];
  case Q:
