...
template <MoveFlag Flag> constexpr Move encode(Square src, Square dst) {
  return Move(src | dst << 6 | Flag);
}

constexpr Square src(Move move) { return static_cast<Square>(move & SrcMask); }
constexpr Square dst(Move move) {
  return static_cast<Square>((move & DstMask) >> 6);
}
constexpr MoveFlag flag(Move move) {
  return static_cast<MoveFlag>(move & FlagMask);
}
...
constexpr std::string format(Move move, bool chess960) {
  ...
  if (!chess960 && is_castle(move))
    dst_sq =
        make_square(flag(move) == QueenCastle ? FileC : FileG, rank_of(src_sq));

  std::string move_str =
      IOUtils::format_sq(src_sq) + IOUtils::format_sq(dst_sq);

  if (is_promo(move))
    move_str += "pnbrqk "[promoted_pt(move)];
