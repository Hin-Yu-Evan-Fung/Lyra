...
constexpr void enum_pawn(const Board &board, Handler &&handler) {
  ...
  if constexpr (Gt & GenCap) {
    ...
    if (ep != NoSquare) {
      BB ep_target   = shift<Down>(from(ep));
    ...
    if ((can_cap_west | can_cap_east) & promo_rank) {
      bitloop(can_cap_west & promo_rank,
              [&](Square src) { enum_promotions<Us, true>(src, shift<UpWest>(src), handler); });
      bitloop(can_cap_east & promo_rank,
              [&](Square src) { enum_promotions<Us, true>(src, shift<UpEast>(src), handler); });
      bitloop(can_cap_west & ~promo_rank,
              [&](Square src) { handler(encode<Cap>(src, shift<UpWest>(src))); });
      bitloop(can_cap_east & ~promo_rank,
              [&](Square src) { handler(encode<Cap>(src, shift<UpEast>(src))); });
    } else {
  ...
  if constexpr (Gt & GenQuiet) {
    ...
    if (can_push & promo_rank) {
      bitloop(can_push & promo_rank,
              [&](Square src) { enum_promotions<Us, false>(src, shift<Up>(src), handler); });
      bitloop(can_push & ~promo_rank,
              [&](Square src) { handler(encode<Quiet>(src, shift<Up>(src))); });
    } else
      bitloop(can_push, [&](Square src) { handler(encode<Quiet>(src, shift<Up>(src))); });

    bitloop(can_double_push,
            [&](Square src) { handler(encode<DoublePush>(src, shift<Up>(shift<Up>(src)))); });
  }
}
...
constexpr void enum_knights(const Board &board, Handler &&handler) {
  ...
  bitloop(moveable, [&](Square src) {
    ...
    if constexpr (Gt & GenCap)
      bitloop(attacks & enemy, [&](Square dst) { handler(encode<Cap>(src, dst)); });
    if constexpr (Gt & GenQuiet)
      bitloop(attacks & empty, [&](Square dst) { handler(encode<Quiet>(src, dst)); });
  });
}
...
constexpr void enum_king(const Board &board, Handler &&handler) {
  ...
  if constexpr (Gt & GenCap)
    bitloop(attacks & enemy, [&](Square dst) { handler(encode<Cap>(ksq, dst)); });
  if constexpr (Gt & GenQuiet)
    bitloop(attacks & empty, [&](Square dst) { handler(encode<Quiet>(ksq, dst)); });
}
