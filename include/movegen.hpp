...
constexpr void enum_pawn(const Board &board, Handler &&handler) {
  ...
  const BB diag_pin = board.undo()->diag_pin;
  const BB hv_pin = board.undo()->hv_pin;
  const BB check_mask = board.undo()->check_mask;
  const Square ep = board.undo()->ep;
  ...
  if constexpr (Gt & GenCap) {
    ...
    if (ep != NoSquare) {
      ...
      if (can_ep_west)
        handler(encode<EP>(lsb(can_ep_west), ep));

      BB can_ep_east = can_cap_east & shift<E>(ep_target & check_mask);
      if (can_ep_east)
        handler(encode<EP>(lsb(can_ep_east), ep));
    }
    ...
    if ((can_cap_west | can_cap_east) & promo_rank) {
      ...
      bitloop(can_cap_west & ~promo_rank, [&](Square src) {
        handler(encode<Cap>(src, shift<UpWest>(src)));
      });
      bitloop(can_cap_east & ~promo_rank, [&](Square src) {
        handler(encode<Cap>(src, shift<UpEast>(src)));
      });
    } else {
      bitloop(can_cap_west, [&](Square src) {
        handler(encode<Cap>(src, shift<UpWest>(src)));
      });
      bitloop(can_cap_east, [&](Square src) {
        handler(encode<Cap>(src, shift<UpEast>(src)));
      });
    }
  }
  ...
  if constexpr (Gt & GenQuiet) {
    BB semi_pushable = pawns & ~diag_pin & shift<Down>(empty) &
                       (~hv_pin | shift<Down>(hv_pin));
    BB can_double_push = semi_pushable & push_rank &
                         shift<Down>(shift<Down>(empty & check_mask));
    BB can_push = semi_pushable & shift<Down>(check_mask);

    if (can_push & promo_rank) {
      bitloop(can_push & promo_rank, [&](Square src) {
        enum_promotions<Us, false>(src, shift<Up>(src), handler);
      });
      bitloop(can_push & ~promo_rank,
              [&](Square src) { handler(encode<Quiet>(src, shift<Up>(src))); });
    } else
      bitloop(can_push,
              [&](Square src) { handler(encode<Quiet>(src, shift<Up>(src))); });

    bitloop(can_double_push, [&](Square src) {
      handler(encode<DoublePush>(src, shift<Up>(shift<Up>(src))));
    });
  }
}
...
constexpr void enum_knights(const Board &board, Handler &&handler) {
  ...
  bitloop(moveable, [&](Square src) {
    ...
    if constexpr (Gt & GenCap)
      bitloop(attacks & enemy,
              [&](Square dst) { handler(encode<Cap>(src, dst)); });
    if constexpr (Gt & GenQuiet)
      bitloop(attacks & empty,
              [&](Square dst) { handler(encode<Quiet>(src, dst)); });
  });
}
...
constexpr void enum_king(const Board &board, Handler &&handler) {
  ...
  const BB attacked = board.undo()->attacked;
  const BB enemy = board.bb(Them);
  const BB empty = ~board.bb();
  ...
  if constexpr (Gt & GenCap)
    bitloop(attacks & enemy,
            [&](Square dst) { handler(encode<Cap>(ksq, dst)); });
  if constexpr (Gt & GenQuiet)
    bitloop(attacks & empty,
            [&](Square dst) { handler(encode<Quiet>(ksq, dst)); });
}
...
constexpr void enum_castle(const Board &board, Handler &&handler) {
  ...
  const Castle cr = board.undo()->c_rights;
  const Square ksq = board.ksq<Us>();
  const Square queen_rs = board.castle_mask().rook_src<Us>(true);
  const Square king_rs = board.castle_mask().rook_src<Us>(false);

  if (cr & OO && board.can_castle<Us, false>())
    handler(encode<KingCastle>(ksq, king_rs));
  if (cr & OOO && board.can_castle<Us, true>())
    handler(encode<QueenCastle>(ksq, queen_rs));
}
...
constexpr void enum_moves(const Board &board, Handler &&handler) {
  ...
  if (!check_mask)
    ...
    if constexpr (Gt & GenQuiet)
      if (check_mask == FullBB)
        enum_castle<Us>(board, handler);
  }
}
...
constexpr std::vector<Move> list_moves(const Board &board) {
  ...
  board.stm() == White ? enum_moves<White, GenAll>(board, append)
                       : enum_moves<Black, GenAll>(board, append);
