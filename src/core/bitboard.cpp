...
template <Direction D1, Direction D2, Direction... Ds> constexpr BB ray(Square sq, BB occ) {
...
template <PieceType pt> void init_magics(Square sq, BB table[], Magic entries[]) {
  static int offset = 0;
...
void init() {
  for (Square src = A1; src <= H8; ++src) {
    for (Square dst = A1; dst <= H8; ++dst) {
      if (ray_attack<B>(src, EmptyBB) & from(dst)) {
        LINE_BB[src][dst] = ray_attack<B>(src, EmptyBB) & ray_attack<B>(dst, EmptyBB);
        BTWN_BB[src][dst] = ray_attack<B>(src, from(dst)) & ray_attack<B>(dst, from(src));
      }
      if (ray_attack<R>(src, EmptyBB) & from(dst)) {
        LINE_BB[src][dst] = ray_attack<R>(src, EmptyBB) & ray_attack<R>(dst, EmptyBB);
        BTWN_BB[src][dst] = ray_attack<R>(src, from(dst)) & ray_attack<R>(dst, from(src));
      }
    }
  }
  ...
  for (Square sq = A1; sq <= H8; ++sq) {
    ...
    KNIGHT_ATK[sq]      = shift<N>(shift<NE, NW>(from(sq))) | shift<E>(shift<NE, SE>(from(sq)))
