...
template <Colour Us>
MovePicker<Us>::MovePicker(const Board &board, Killer *killer,
                           MainHistory *history, Move tt_move, Depth depth)
    : board_(board), killer_(killer), history_(history), tt_move_(tt_move),
      depth_(depth) {
  stage_ = MAIN_TT;
}
...
template <Colour Us>
MovePicker<Us>::MovePicker(const Board &board, Killer *killer,
                           MainHistory *history, Move tt_move)
    : board_(board), killer_(killer), history_(history), tt_move_(tt_move),
      depth_(0) {
  if (board_.in_check())
    stage_ = EVASION_TT;
