...
template <Colour Us>
MovePicker<Us>::MovePicker(const Board &board, const Killer &killer,
                           const MOStats &stats, Move tt_move, Depth depth)
    : board_(board), killer_(killer), stats_(stats), tt_move_(tt_move),
      depth_(depth) {
  stage_ = MAIN_TT;
}
...
template <Colour Us>
MovePicker<Us>::MovePicker(const Board &board, const Killer &killer,
                           const MOStats &stats, Move tt_move)
    : board_(board), killer_(killer), stats_(stats), tt_move_(tt_move),
      depth_(0) {
  if (board_.in_check())
    stage_ = EVASION_TT;
...
template <Colour Us> size_t MovePicker<Us>::best_idx(size_t start, size_t end) {
  return std::distance(scores_,
                       std::max_element(scores_ + start, scores_ + end));
}
...
template <Colour Us> Eval MovePicker<Us>::score_quiet(Move move) {
  return stats_.ht.get(board_, move);
}
...
template <Colour Us> void MovePicker<Us>::gen_score_quiet() {
  ...
  enum_moves<Us, GenQuiet>(board_, [&](Move move) {
    if (move == tt_move_ || move == killer_.moves[0] ||
        move == killer_.moves[1])
      return;
...
template <Colour Us> Move MovePicker<Us>::next() {
  ...
  case KILLER_1:
    ...
    if (killer != tt_move_ && board_.is_legal<Us>(killer))
      return killer;
    [[fallthrough]];
  case KILLER_2:
    ++stage_;
...
template <Colour Us> bool Board::is_legal(Move move) const {
  ...
  const bool invalid_cap_flag =
      !is_castle && !is_ep && is_cap == (cap == NoPiece);

  if (invalid_pc || invalid_cap || invalid_sq || invalid_cap_flag)
    return false;

  if (is_castle) {
    if (flag == KingCastle && undo_->c_rights & OO)
      return !in_check() && can_castle<Us, false>();
    if (flag == QueenCastle && undo_->c_rights & OOO)
      return !in_check() && can_castle<Us, true>();
    return false;
  }

  if (pt == K)
    return KING_ATK[src] & from(dst) && !(attacked & from(dst));

  // Check if there is a pin and if so check if the movement is aligned to the
  // king
  const bool valid_pin = !(diag_pin & from(src) || hv_pin & from(src)) ||
                         is_aligned(src, dst, ksq_);

  if (pt == P) {
    // Enpassant is illegal if the destination is incorrect, the attack pattern
    // is wrong, or the move results in check.
    if (is_ep)
      return undo_->ep == dst && valid_pin &&
             PAWN_ATK[Us][src] & from(dst) & shift<Up>(check_mask);
    // Capture is illegal if the move does not correspond to the attack pattern
    if (is_cap && !(PAWN_ATK[Us][src] & from(dst)))
      return false;
    // Double Push is illegal if there are pieces in the way
    if (is_dp && (BTWN_BB[src][dst] | from(dst)) & occ)
      return false;
    // Single Push is illegal if its the wrong pattern and there are pieces in
    // the way
    if (is_quiet && (dst != forward<Us>(src) || from(dst) & occ))
      return false;
...
inline BB Board::attackers_to(Square to, BB occ) const {
  return (PAWN_ATK[Black][to] & bb(White, P)) |
         (PAWN_ATK[White][to] & bb(Black, P)) | (KNIGHT_ATK[to] & bb(N)) |
         (BISHOP_ATK[to][occ] & bb(B, Q)) | (ROOK_ATK[to][occ] & bb(R, Q)) |
         (KING_ATK[to] & bb(K));
}
...
bool Board::see(Move move, Eval lower_bound) const {
  ...
  const Square ep_target =
      stm_ == White ? shift<Direction::N>(dst) : shift<Direction::S>(dst);
  ...
  if (is_castle(move))
    return 0 <= gain;
  if (is_capture(move))
    gain += flag == EP ? PIECE_VALS[P] : PIECE_VALS[vic];
  if (is_promo(move))
    gain += PIECE_VALS[promo] - PIECE_VALS[att];
  ...
  while (true) {
    ...
    if (lva_pt == P || lva_pt == B || lva_pt == Q)
      atks |= BISHOP_ATK[dst][occ] & diag_sliders;
    if (lva_pt == R || lva_pt == Q)
      atks |= ROOK_ATK[dst][occ] & hv_sliders;
