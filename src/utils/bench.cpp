...
constexpr unsigned N_POS = 66;

constexpr std::string_view bench_fens[N_POS] = {
...
void run_bench(int argc, char **argv) {
  ...
  tc.depth = argc > 2 ? atoi(argv[2]) : 13;
  ...
  for (unsigned i = 0; i < N_POS; ++i) {
    ...
    total_nodes += worker.nodes_;
    total_time += worker.clock_.elapsed();
    total_nps += worker.nodes_ * 1000 / std::max(worker.clock_.elapsed(), 1UL);
  }

  std::println("{} nodes {} nps {} time", total_nodes, total_nps / N_POS, total_time);
}
