...
TEST(is_legal, castling_check) {
  ...
  ASSERT_FALSE(
      board.is_legal<White>(MoveUtils::encode<KingCastle>(E1, H1))); // Castling through check
  ASSERT_FALSE(
      board.is_legal<White>(MoveUtils::encode<QueenCastle>(E1, A1)));   // Castling through check
