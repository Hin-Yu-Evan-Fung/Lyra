...
void perft_bench(BenchTestCase test_cases[], int n_cases) {
  Board board;
  U64 nodes = 0;
  ...
  for (int i = 0; i < n_cases; i++) {
    ...
    std::println("Status: {}, Time: {} ms, NPS: {}, Fen: {}",
                 (nodes == validation) ? "PASSED" : "FAILED", elapsed, nps,
                 board.fen());

    ASSERT_TRUE(nodes == validation);
  }
}
...
TEST(perft, bench_normal) { perft_bench<Perft>(normal, 31); };
TEST(perft, bench_mp_normal) { perft_bench<Perft_MP>(normal, 5); };
...
BenchTestCase chess960[] = {
    ...
    {"1b1qbkrn/1prp1pp1/pn5p/2p1pB2/Q1PP4/1N6/PP2PPPP/2R1BKRN w KQk - 2 9", 6,
     1648762553},
    {"1rkb1qr1/pppp2pp/1n2p1n1/3b1p2/3N3P/P2P1P2/1PP1P1P1/1RKBBQRN w KQkq - 3 "
     "9",
     6, 1042669941},
    {"1b1r1krb/ppp1np2/qn1p2pp/3Bp3/2P1P1PP/1N1P4/PP3P2/1BNRQKR1 w KQkq - 0 9",
     6, 1169912833}};

TEST(perft, bench_chess960) { perft_bench<Perft>(chess960, 15); };
TEST(perft, bench_mp_chess960) { perft_bench<Perft_MP>(chess960, 5); };
